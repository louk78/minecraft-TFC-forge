	...
	for (int iMul = iMaxMultiple; iMul > 0; iMul--)
	{
		...
		else
		{
			...
			if (actioncout.bCan)
			{
				...
				{
					csTemp.Format(_T("%d鐓锻"), actioncout.iThirteenCount);
					csResult += csTemp;
					csTemp = "";
				}
				if (actioncout.iSevenCount != 0)
				{
					csTemp.Format(_T("%d弯曲"), actioncout.iSevenCount);
					csResult += csTemp;
					csTemp = "";
				}
				if (actioncout.iTwoCount != 0)
				{
					csTemp.Format(_T("%d冲压"), actioncout.iTwoCount);
					csResult += csTemp;
					csTemp = "";
				}
				break;

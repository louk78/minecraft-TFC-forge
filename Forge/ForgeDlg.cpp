...
CForgeDlg::CForgeDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CForgeDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CForgeDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
...
BEGIN_MESSAGE_MAP(CForgeDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_MOVE()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_BT_CALCULATE, &CForgeDlg::OnBnClickedBtCalculate)
	ON_BN_CLICKED(IDC_BT_CACULTARGET, &CForgeDlg::OnBnClickedBtCaculForgePoint)
	ON_CBN_SELCHANGE(IDC_CB_ACTION3, &CForgeDlg::OnCbnSelchangeCbAction3)
	ON_CBN_SELCHANGE(IDC_CB_ACTION2, &CForgeDlg::OnCbnSelchangeCbAction2)
	ON_CBN_SELCHANGE(IDC_CB_ACTION1, &CForgeDlg::OnCbnSelchangeCbAction1)
	ON_BN_CLICKED(IDC_CLEANTOZERO, &CForgeDlg::OnBnClickedCleantozero)
	ON_WM_CLOSE()
END_MESSAGE_MAP()
...
BOOL CForgeDlg::OnInitDialog()
{
	...
	TCHAR chActionText[9][5] = { _T("轻击"), _T("击打"), _T("重击"), _T("牵拉"),_T("冲压"), _T("弯曲"), _T("鐓锻"), _T("收缩"),_T("无") };
...
void CForgeDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR CForgeDlg::OnQueryDragIcon()
{
...
void CForgeDlg::OnMove(int x, int y)
{
...
void CForgeDlg::OnSize(UINT nType, int cx, int cy)
{
...
void CForgeDlg::OnBnClickedBtCalculate()
{
	//动作文本表格
	TCHAR chActionText[9][5] = { _T("轻击"), _T("击打"), _T("重击"), _T("牵拉"), _T("冲压"), _T("弯曲"), _T("鐓锻"), _T("收缩"), _T("无") };
	//动作值表格
	...
	for (int iMul = iMaxMultiple; iMul > 0; iMul--)
	{
		...
		if (iRemainer == 0)
		{
			csResult.Format(_T("公式：%d个收缩"), iMul);
			break;
		}
		else
		{
			...
			if (actioncout.bCan)
			{
				...
				{
					csTemp.Format(_T(" + %d个鐓锻"), actioncout.iThirteenCount);
					csResult += csTemp;
					csTemp = "";
				}
				if (actioncout.iSevenCount != 0)
				{
					csTemp.Format(_T(" + %d个弯曲"), actioncout.iSevenCount);
					csResult += csTemp;
					csTemp = "";
				}
				if (actioncout.iTwoCount != 0)
				{
					csTemp.Format(_T(" + %d个冲压"), actioncout.iTwoCount);
					csResult += csTemp;
					csTemp = "";
				}
				break;
...
void CForgeDlg::OnBnClickedBtCaculForgePoint()
{
	// TODO:  在此添加控件通知处理程序代码
...
void CForgeDlg::OnCbnSelchangeCbAction1()
{
	int iCurSel;
...
void CForgeDlg::OnCbnSelchangeCbAction2()
{
	int iCurSel;
...
void CForgeDlg::OnCbnSelchangeCbAction3()
{
	int iCurSel;
...
void CForgeDlg::OnBnClickedCleantozero()
{
	// TODO:  在此添加控件通知处理程序代码
	GetDlgItem(IDC_ET_SHRINK)->SetWindowText(_T("0"));
	GetDlgItem(IDC_ET_UPSET)->SetWindowText(_T("0"));
